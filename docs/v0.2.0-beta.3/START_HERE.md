# BWR-4 Simulator v0.2.0-beta.3 — 처음 시작하기
...
구현과 검증 코드를 직접 살펴보려는 테스터용 공개 소스 스냅샷은 별도 페이지에
있다. 이 저장소의 자동 `Source code` 압축에는 전체 Python 원본이 들어 있으며,
자유 재배포형 오픈소스가 아니라 source-available 베타 라이선스를 적용한다.

https://github.com/Loun0521/bwr4-simulator-source-beta/releases/tag/source-v0.2.0-beta.3
...
```bash
tar -xzf BWR4-Simulator-v0.2.0-beta.3-Linux-x64.tar.gz
cd BWR4-Simulator-v0.2.0-beta.3-Linux-x64
./BWR4-Simulator
```
...
실행 압축의 `OPEN_DOCUMENTATION.html`을 열면 기본 웹 브라우저가 이 GitHub 안내
페이지를 연다. 초보자용 실행법, 물리 모델, 검증 보고서와 R-104B/R-304B 검색
자료는 모두 GitHub 안에서 볼 수 있다. 로컬 `.md` 연결 프로그램이 Claude나 다른
편집기로 지정되어 있어도 영향을 받지 않는다.

실행 압축에는 사용자 문서 복사본을 넣지 않는다. 사용 조건은 `LICENSE.txt`, 필수
제3자 고지는 `THIRD_PARTY_LICENSES/`에 텍스트로 남겨 오프라인에서도 확인할 수
있다. 개발 변경 이력, 구현 계획, 테스트 내부 보고서와 저장소 관리 문서도 실행
배포본에 넣지 않는다.
