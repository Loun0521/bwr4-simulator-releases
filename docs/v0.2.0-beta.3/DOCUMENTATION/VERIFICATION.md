...
### 13.40 Separate source-available public beta (2026-09-20)

The private development repository and binary-only public distribution repository remain
separate. A new public repository, `Loun0521/bwr4-simulator-source-beta`, starts from a
clean snapshot rather than the private Git history. The snapshot copied all 233 files
tracked at private source commit `bab2032c0e9720a5ac84f7368c3f9d35c9a1a223` and added
four public-repository documents plus a validation workflow. Its first commit therefore
tracked 238 files. Ignored virtual environments, caches, simulator state, test results,
build output, and credentials were not copied. A pre-publication scan found no tracked
GitHub token, cloud key, or private-key pattern, and the largest tracked source file was
0.43 MiB.

The public overlay preserves the private-repository README and compiled-beta license as
`PROJECT_README.md` and `BINARY_BETA_LICENSE.md`. The replacement README and license
identify the repository as source-available rather than OSI open source. They allow
personal non-commercial inspection, local execution, testing, local modification, and
official-repository contributions while prohibiting independent redistribution, mirrors,
published modified builds, sales, and hosted or commercial services. `CONTRIBUTING.md`
states reproducibility and submission-license requirements; `SOURCE_SNAPSHOT.md` records
the exact private source commit and exclusions.

Unlike the binary distribution repository, the source-beta copy removes private-source
`export-ignore` rules. `tests/release_contracts.py` detects `SOURCE_SNAPSHOT.md` and checks
the full-source archive policy there while retaining the restricted archive assertions in
the private source role. The adjusted release-contract module passed locally:

```text
python -m unittest tests.release_contracts -v
10 tests, 0.198 s, exit 0
```

An attempted full fast run under the bundled Codex Python runtime was not counted as a
pass. It produced 36 harness passes and one process-isolation ERROR because the Windows
child process tree did not terminate after its timeout; the contracts worker then reached
the outer timeout. The run exited 2 after 315.15 seconds. The original `.venv` could not
be used because its recorded base Python installation no longer existed. This is retained
as an environment-specific incomplete run rather than reported as regression success.

The same bounded gate passed twice on clean GitHub-hosted Windows runners:

```text
GitHub Actions 35513088626, commit 4de08f3
dependency installation                    PASS, 0m19s
contracts + harness bounded fast gate      PASS, 3m21s

GitHub Actions 35513544956, commit b3dcfc8
dependency installation                    PASS, 0m14s
contracts + harness bounded fast gate      PASS, 3m24s
```

Final commit `b3dcfc8` was tagged `source-v0.2.0-beta.1` and published as a non-draft
pre-release. An unauthenticated download of the tag archive was 2,404,091 bytes and held
249 tar entries, including 86 Python files, `core/bwr4_core.py`, `LICENSE.md`, and
`SOURCE_SNAPSHOT.md`. The release intentionally has no uploaded binary assets; GitHub's
generated Source code archives are the full public snapshot. The binary release remains
at `Loun0521/bwr4-simulator-releases` and its generated source archive still contains only
the distribution README and license.

Not run specifically for the source-publication overlay: integration, the six-case GUI
profile, numeric reports, NMS full startup, or 30-day stress. Physics, plant-system code,
...
### 13.42 macOS LaunchServices argument regression (2026-09-21)

A macOS tester supplied this beta.2 startup failure from
`~/Library/Application Support/BWR4Simulator/logs/startup-error.log`:

```text
ValueError: invalid literal for int() with base 16: '--smoke-test'
  File "bwr4_simulator.py", line 59, in main
    seed = int(args[0], 16) if args else None
```

The entry point recognized the packaged smoke path only when the complete argument list
was exactly `['--smoke-test']`. macOS LaunchServices can append a private process serial
number argument such as `-psn_0_123456` when launching a GUI application. The resulting
`['--smoke-test', '-psn_0_123456']` missed the exact-list branch, and the generic path
then attempted to parse `--smoke-test` as a hexadecimal core seed. A normal Finder launch
with only `-psn_…` could reach the same invalid seed conversion.

`_application_args()` now removes only `-psn_`-prefixed arguments and only when
`sys.platform == 'darwin'`, before interpreting the documented arguments. `_run_panel()`
separates ordinary panel startup so the no-seed Finder route can be tested without making
a real window. The regression contract covers all of the following:

- `['--smoke-test', '-psn_0_123456']` calls `smoke_test()` and returns 0;
- `['-psn_0_123456']` starts the panel with `seed=None`;
- Linux retains the same string instead of silently accepting a macOS-private argument;
  and
- the macOS packaged smoke command itself includes synthetic `-psn_0_1`, closing the CI
  blind spot caused by directly launching `.app/Contents/MacOS/BWR4-Simulator`.

The release number advances to `0.2.0-beta.3`; the published beta.2 assets and checksums
remain immutable. Local focused verification was:

```text
python -m unittest tests.release_contracts -v
12 tests, 0.256 s, exit 0

python -m py_compile bwr4_simulator.py packaging/build_beta.py \
  tests/release_contracts.py core/version.py
exit 0
...
GitHub Actions run
[`35519605140`](https://github.com/Loun0521/bwr4-simulator/actions/runs/35519605140)
completed successfully for source commit `6394ee31bff3e03f9655ba6722d103409ed722f8`.
The bounded fast verification passed in 2 min 48 s. All native jobs passed: Linux arm64
in 56 s, Linux x64 in 1 min 13 s, macOS arm64 in 44 s, macOS x64 in 1 min 35 s, and
Windows x64 in 1 min 1 s. The release job was intentionally skipped because the run was
manually dispatched.

Both macOS job logs show the packaged executable, rather than only source-level code,
running successfully with the formerly failing argument shape:

```text
BWR4-Simulator.app/Contents/MacOS/BWR4-Simulator --smoke-test -psn_0_1
```

The five downloaded archives were then inspected independently. Every archive sidecar
matched, every executable and required license was present, every bundle had exactly one
`OPEN_DOCUMENTATION.html` pointing at the beta.3 tag, and there were zero user-facing
Markdown files. The nine internal NumPy `*.dist-info/licenses/**/LICENSE.md` files remain
third-party license originals as documented in §13.41.

| archive | bytes | SHA-256 |
|---|---:|---|
| Linux arm64 | 54,445,003 | `94ee36cc25762bd0cc0648e4bf0e0f39cf258bfa3147eaac9151fac14b7d53ae` |
| Linux x64 | 56,238,613 | `5a5588d852deaaa5da68edd2dfb7118adef83f4f693b24e09fe7ff4f68b095db` |
| macOS arm64 | 18,298,103 | `a28a06d28441d5b141e363af003e23c867a065838f59d8687a7e7ef4d782e8a7` |
| macOS x64 | 19,761,182 | `d3ce5550f1ce7ec07876d353fda4d2ae9ed70bfa485a82b835130ca7a43c238a` |
| Windows x64 | 26,422,972 | `ea7f43ac1f5d563815dd0389a38fa5b5c56a001b84046b8a7dbcc4870e4c47d8` |

The inspected binaries are published as
[`v0.2.0-beta.3`](https://github.com/Loun0521/bwr4-simulator-releases/releases/tag/v0.2.0-beta.3),
and the matching full source snapshot is published as
[`source-v0.2.0-beta.3`](https://github.com/Loun0521/bwr4-simulator-source-beta/releases/tag/source-v0.2.0-beta.3).
